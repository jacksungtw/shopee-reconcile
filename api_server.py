...
def _fix_filename(name: str) -> str:
    """還原 multipart 上傳時被 latin1 誤解的 UTF-8 中文檔名。"""
    if not name:
        return name
    try:
        # 若檔名能用 latin1 編回再以 utf-8 解出，表示原本是被 latin1 誤讀的 UTF-8
...
async function run(){
  ...
  try{
    ...
    if(mv){
      body.month=parseInt(mv);
    }else{
      // 沒選月份 → 從紙本 CSV 檔名抓（瀏覽器端檔名是正確 UTF-8，最可靠）
      const csvFile=files.find(f=>f.name.toLowerCase().endsWith('.csv'));
      if(csvFile){const m=csvFile.name.match(/(\\d{1,2})\\s*月/);if(m)body.month=parseInt(m[1]);}
    }
...
async def upload(file: UploadFile = File(..., description="Excel (.xlsx) 或紙本 CSV"),
    ...
    fname = _fix_filename(file.filename)
    target = session_dir / fname
    with open(target, "wb") as out:
        shutil.copyfileobj(file.file, out)
    return UploadResp(file_id=file_id, filename=fname,
